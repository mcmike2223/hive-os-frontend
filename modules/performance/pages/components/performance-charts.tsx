...
export function PerformanceCharts({ data, focus = "full" }: { data: PerformanceDashboard; focus?: "full" | "team" | "reports" }) {
  const datasets = data.datasets;
  return <section aria-label="Performance analytics" className="grid min-w-0 gap-6 xl:grid-cols-2">
    {focus !== "reports" ? <ChartCard title="Review pipeline" description="Reviews by current workflow stage; use this to find hand-off bottlenecks." label="Bar chart of performance reviews grouped by workflow status. A detailed data table follows.">
    ...
    {focus === "full" || focus === "reports" ? <ChartCard title="Score distribution" description="Completed review outcomes grouped into the configured performance bands." label="Bar chart showing completed review scores by rating band. A detailed data table follows.">
    ...
    {focus === "reports" ? <ChartCard title="Goal health" description="Goals grouped by status to highlight delivery risk in the selected scope." label="Bar chart of performance goals grouped by status. A detailed data table follows.">
      <ResponsiveContainer width="100%" height={280}><BarChart data={datasets.goal_status} accessibilityLayer margin={{ top: 12, right: 12, left: 0 }}><CartesianGrid vertical={false} strokeDasharray="4 4" /><XAxis dataKey="label" tickLine={false} axisLine={false} interval={0} tick={{ fontSize: 11 }} /><YAxis allowDecimals={false} tickLine={false} axisLine={false} width={32} /><Tooltip /><Bar dataKey="value" name="Goals" fill={chartColors.warning} radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer>
      <PerformanceTable caption="Goal count by status." rows={datasets.goal_status} getKey={(row) => row.status} columns={[{ key: "status", label: "Goal status", render: (row) => row.label }, { key: "value", label: "Goals", align: "right", render: (row) => row.value }]} />
    </ChartCard> : null}
    {focus === "full" || focus === "reports" ? <ChartCard title="Check-in rhythm" description="Planned and completed conversations over the most recent six months." label="Line chart comparing planned and completed performance check-ins by month. A detailed data table follows.">
