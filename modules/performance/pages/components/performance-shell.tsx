...
export function PerformanceTable<T>({ caption, columns, rows, getKey, rowRef }: { caption: string; columns: Array<{ key: string; label: string; align?: "left" | "right"; render: (row: T) => ReactNode }>; rows: T[]; getKey: (row: T) => number | string; rowRef?: (row: T, node: HTMLTableRowElement | null) => void }) { if (!rows.length) return <PerformanceEmpty title="No records yet" description={caption} />; return <div className="w-full overflow-x-auto"><Table><TableCaption>{caption}</TableCaption><TableHeader><TableRow>{columns.map((column) => <TableHead key={column.key} scope="col" className={column.align === "right" ? "text-right" : undefined}>{column.label}</TableHead>)}</TableRow></TableHeader><TableBody>{rows.map((row) => <TableRow key={getKey(row)} ref={(node) => rowRef?.(row, node)}>{columns.map((column) => <TableCell key={column.key} className={column.align === "right" ? "text-right" : undefined}>{column.render(row)}</TableCell>)}</TableRow>)}</TableBody></Table></div>; }
