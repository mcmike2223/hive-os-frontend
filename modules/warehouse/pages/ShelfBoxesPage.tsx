...
const readPayloadValue = (
  record: WarehouseLocation | Record<string, unknown>,
  key: string,
...
const toStorableListItemFromProduct = (
...
const toStorableListItemFromInventoryItem = (
  item: InventoryItem,
): StorableListItem => ({
  id: item.id,
  name: item.name,
...
export function ShelfBoxesPage() {
  ...
  const [filterOccupied, setFilterOccupied] = React.useState<
  ...
  const productsQuery = useQuery({
    ...
    queryFn: async (): Promise<Record<number, ProductRecord>> => {
      ...
      const results = await Promise.all(
        allProductIds.map((id) => fetchInventoryProduct(id)),
      );
  ...
  const saveMutation = useMutation({
    mutationFn: async (data: any) => {
      ...
      return warehouseApi.createLocation({
        ...data.payload,
    ...
    onSuccess: (response: any) => {
      ...
      const outcome = notifyMutationOutcome(body, {
        savedMessage: addProductId
          ? t(
              "inventory.shelf_boxes.assigned",
              "Product assigned to shelf successfully.",
            )
    ...
    onError: (err: any) => {
      toast.error(
        err?.response?.data?.message ??
          t("inventory.common.failed", "Error saving box."),
      );
    },
  });
  ...
  const occupiedCount = boxesDataAll.filter(
    (b: any) => b.metadata?.status === "occupied",
  ).length;
  const availableCount = boxesDataAll.filter(
    (b: any) => b.metadata?.status !== "occupied",
  ).length;
  const totalStoredQty = boxesDataAll.reduce(
    (sum: number, b: any) => sum + (Number(b.metadata?.quantity_stored) || 0),
    0,
  );

  const occupiedBoxes = boxesDataAll.filter(
    (b: WarehouseLocation) =>
      b.metadata?.status === "occupied" &&
      b.metadata?.storable_type === "product",
  );
    ...
    return (
      <div className="flex h-[400px] flex-col items-center justify-center p-8 text-center bg-muted/20 border border-border/40 rounded-3xl m-8">
        ...
        <h2 className="text-xl font-bold mb-2">
          {t("warehouse.boxes.select_shelf", "Select a shelf to view boxes")}
        </h2>
        <p className="text-muted-foreground mb-6 max-w-md mx-auto">
          {t(
            "warehouse.boxes.select_shelf_desc",
            "Boxes are managed visually within their parent shelves. Return to the Shelves page and click 'Manage Boxes' on a specific shelf.",
          )}
        </p>
        <Button
  ...
  if (!shelf)
    return (
      <div className="p-8 text-center text-muted-foreground">
        {t("inventory.shelves.not_found", "Shelf not found")}
      </div>
    );
    ...
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
        <div className="space-y-4">
          <Button
            ...
            onClick={() =>
              router.push("/dashboard/warehouse/locations/shelves")
            }
          >
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-sm font-medium text-muted-foreground">
                  {t("warehouse.title", "Warehouse")}:{" "}
                  {shelf.warehouse?.name ||
                    t("inventory.common.none", "Unassigned")}
                </span>
              </div>
              <h1 className="text-3xl font-black tracking-tight underline decoration-primary/30 underline-offset-4">
                {t("inventory.shelves.manage_shelf", "Manage Shelf")}:{" "}
                <span className="text-primary">{shelf.name || shelf.code}</span>
              </h1>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-2 lg:justify-end">
          <Badge
            variant="outline"
            className="rounded-full bg-background/50 px-3 py-1 font-mono text-xs backdrop-blur-sm border-primary/20"
          >
            {shelf.code}
          </Badge>
        </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-muted-foreground">
                Click on any box below to assign this product to it.
              </p>
            </div>
            <Button
              variant="ghost"
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          <div
            key={i}
            className="group relative overflow-hidden rounded-[2rem] border border-border/50 bg-background/40 p-6 backdrop-blur-xl transition-all hover:border-primary/30"
          >
            <div
              className={`mb-3 flex h-10 w-10 items-center justify-center rounded-xl bg-background/80 shadow-sm border border-border/10 ${stat.color}`}
            >
      ...
      <div className="flex flex-wrap items-center gap-6 rounded-[1.5rem] border border-border/40 bg-muted/20 px-6 py-4 backdrop-blur-sm">
        <span className="text-xs font-bold uppercase tracking-widest text-muted-foreground/60">
          {t("inventory.common.legend", "Legend")}
        </span>
        {[
          {
            color: "bg-background border-border",
            label: t("inventory.shelf_boxes.available", "Available"),
          },
          {
            color: "bg-primary border-primary/30",
            label: t("inventory.shelf_boxes.occupied", "Occupied"),
          },
          {
            color: "bg-orange-500 border-orange-500/30",
            label: t("inventory.shelf_boxes.reserved", "Reserved"),
          },
          ...
          <div key={i} className="flex items-center gap-2">
            {item.color && (
              <div
                className={`h-4 w-4 rounded-md border shadow-sm ${item.color}`}
              />
            )}
            {item.icon && (
        ...
        {Array.from({ length: rows }).map((_, rowIndex) =>
          Array.from({ length: columns }).map((_, colIndex) => {
            ...
            const statusValue = String(
              readPayloadValue(
                (box || {}) as WarehouseLocation,
                "status",
                "available",
              ) ?? "available",
            );
            const storableType = readPayloadValue(
              (box || {}) as WarehouseLocation,
              "storable_type",
            );
            ...
            if (!matchesSearch || !matchesFilter) {
              return (
                <div
                  key={`${r}-${c}`}
                  className="aspect-video rounded-[1.5rem] border border-dashed border-transparent"
                />
              );
            }
            ...
            return (
              ...
              >
                ...
                {box ? (
                  <>
                    <div
                      className={`mb-2 rounded-xl p-2 transition-transform group-hover:scale-110 ${statusValue === "occupied" ? "bg-primary/20 text-primary dark:text-primary-foreground" : statusValue === "reserved" ? "bg-orange-500/20 text-orange-600 dark:text-orange-400" : "bg-muted text-muted-foreground"}`}
                    >
                      {storableTypeMeta === "product" ? (
...
function BoxAssignmentDialog({
  ...
  const selectedProduct = React.useMemo((): StorableListItem | null => {
    ...
    const fromList = productsQuery.data?.find(
      (p) => p.id === Number(formData.storable_id),
    );
  ...
  React.useEffect(() => {
    if (boxData?.record) {
      ...
      const existingStatus = String(
        readPayloadValue(box, "status", "available") ?? "available",
      );
      ...
      setFormData({
        ...
        storable_type: shouldPrefillAssign
          ? "product"
          : String(
              readPayloadValue(box, "storable_type", "product") ?? "product",
            ),
      ...
      setFormData({
        name: boxData
          ? `${t("inventory.shelf_boxes.box", "Box")} R${boxData.row}C${boxData.col}`
          : "",
  ...
  React.useEffect(() => {
    ...
    if (
      !existingStorableId ||
      String(existingStorableId) === String(addProductId)
    ) {
        ...
        <div className="border-b border-border/20 px-8 py-6">
          <DialogHeader>
            <DialogTitle className="text-2xl font-black tracking-tight">
              {boxData?.record
                ? t("inventory.shelf_boxes.edit_title", "Edit Box")
                : t("inventory.shelf_boxes.create_title", "Create Box")}{" "}
              - R{boxData?.row}C{boxData?.col}
            </DialogTitle>
            <DialogDescription>
              {t(
                "inventory.shelf_boxes.config_desc",
                "Configuration for storage coordinates",
              )}{" "}
              R{boxData?.row} {t("inventory.common.and", "and")} C{boxData?.col}
              .
            </DialogDescription>
          </DialogHeader>
        </div>
        ...
        <div className="grid gap-6 px-8 py-8 md:grid-cols-2">
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80">
              {t("inventory.shelf_boxes.name_label", "Box Name")}
            </Label>
            <Input
              value={formData.name}
              onChange={(e) =>
          ...
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80">
              {t("inventory.shelf_boxes.quantity_label", "Stored Qty")}
            </Label>
            <Input
              type="number"
              value={formData.quantity_stored}
              onChange={(e) =>
                setFormData((p) => ({ ...p, quantity_stored: e.target.value }))
              }
              className="rounded-2xl border-border/40 bg-background/50"
            />
          </div>
          <div className="space-y-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80">
              {t("inventory.shelf_boxes.storable_type", "Storable Type")}
            </Label>
            <Select
              value={formData.storable_type}
              onValueChange={(v) =>
                setFormData((p) => ({ ...p, storable_type: v }))
              }
            >
              <SelectTrigger className="rounded-2xl border-border/40">
                <SelectValue />
              ...
              <SelectContent className="rounded-2xl">
                <SelectItem value="product">
                  {t("inventory.common.product", "Product")}
                </SelectItem>
                <SelectItem value="good">
                  {t("inventory.common.good", "Good / Item")}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-2 md:col-span-2">
            <Label className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80">
              {t("inventory.common.notes", "Notes")}
            </Label>
            <Textarea
              ...
              placeholder={t(
                "inventory.shelf_boxes.notes_placeholder",
            ...
            <div className="flex-1">
              ...
              >
                <AlertDialogTrigger asChild>
                  ...
                  >
                    <Loader2
                      className={`mr-2 h-4 w-4 ${clearBoxMutation.isPending ? "animate-spin" : ""}`}
                    />
                    Clear Box
                  </Button>
                </AlertDialogTrigger>
